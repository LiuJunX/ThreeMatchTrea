...
**"让玩家觉得自己聪明"**——好的关卡不是靠运气通关，而是让玩家在复盘时觉得"我的选择导致了胜利"。

核心设计思路：
1. **每关有一个"核心谜题"**：不是堆砌元素，而是围绕一个设计意图组织
2. **多条通关路径**：不依赖唯一解法或唯一运气事件
3. **渐进复杂度**：新机制逐步引入，每步都让玩家有"啊哈"时刻
4. **难度在于规划，而非运气**：步数紧张但可计算，不是等某种颜色出现

### 15.2 关卡设计三问

设计每一关前问自己：
1. **这关教了什么或考了什么？**（教学/检验哪个机制）
2. **玩家的"啊哈时刻"是什么？**（发现某种策略/组合/路径）
3. **如果失败，玩家知道为什么吗？**（可归因 vs 随机）

### 15.3 元素组合的设计思路

| 设计意图 | 推荐组合 | 避免 |
|----------|---------|------|
| 教学：认识新元素 | 单一新元素 + 低 HP + 大量步数 | 多种新元素同时出现 |
| 策略：颜色规划 | ColorBox + 收集目标 | 7 色 + ColorBox |
| 节奏：爽快感 | 4 色 + Bush(→Grass 连锁) | 太多免疫匹配障碍 |
| 挑战：道具制造 | Safe/Stone + 宽棋盘 | 道具需求 + 窄棋盘 |
| 复杂：多目标 | 同向双目标 | 互相矛盾的目标 |
...
| 错误 | 后果 | 检测方法 |
|------|------|---------|
| 目标不可完成 | 无限重试 | 分析管线胜率 = 0 |
| 开局无解（死锁） | 立即流失 | 分析管线死锁率 > 10% |
| 颜色饥荒 | 目标色不生成 | 检查 Spawner 配置 |
| 障碍堵死 Spawner | 无法生成新方块 | Generator 已避免 |
| Honey 无可操作相邻格 | 永远无法清除 | 手动检查 |

### 16.2 体验问题（应该避免）

| 问题 | 症状 | 解决 |
|------|------|------|
| 纯运气通关 | 胜率方差极大 | 减颜色数、增步数 |
| 开局无事可做 | 等待生成特定颜色 | 预放有利初始布局 |
| 目标不可见 | 玩家不知道该做什么 | 目标元素放在可见区域 |
| 道具需求但无法制造 | Safe 旁无 4 消空间 | 检查道具制造可行性 |

### 16.3 元素搭配禁忌

| 组合 | 问题 |
|------|------|
| Honey + Cage 同区域 | 双重封锁，几乎不可操作 |
| Safe + Stone + Owl 同关 | 道具需求灾难 |
| 7 色 + ColorBox/PotionBottle | 颜色匹配概率过低 |
| Chain + Frost 大量混用 | 功能重叠，无差异化 |
| Mailbox ×3+ | Envelope 泛滥堵塞棋盘 |

### 16.4 自动化关卡的迭代流程

```
1. Generator 按蓝图生成 LevelConfig + design.md
2. 分析管线跑 1000 次模拟 → 胜率/死锁率
3. 对比目标胜率范围：
   - 太难 → 加步数(±2) → 重新分析
   - 太简单 → 减步数(±2) → 重新分析
   - 死锁率 >5% → 检查棋盘形状/障碍放置
4. 通过 → 保存 JSON + design.md
5. 在 design.md 的"待迭代"区记录分析结果
6. 实际游玩测试后在"设计发现"区记录新想法
```
...
本文档为关卡设计的顶层指南。更详细的元素级设计知识见：

**`docs/03-design/level-knowledge/`** — 关卡设计知识库

- `core/` — 核心机制（匹配、掉落、炸弹、难度调节）
- `elements/` — 每个元素的放置原则、组合建议、常见错误
- `patterns/` — 设计模板（教学关、Boss 关、双目标）

Generator 生成关卡时会自动引用对应的知识文档。
