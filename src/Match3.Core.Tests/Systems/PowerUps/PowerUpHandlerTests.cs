...
public class PowerUpHandlerTests
{
    ...
    public void ActivateBomb_VerticalBomb_ClearsColumn()
    {
        ...
        for (int y = 0; y < 8; y++)
        {
            Assert.Equal(ElementType.None, state.GetTile(3, y).Type);
        }
    }
    ...
    public void ActivateBomb_ColorBomb_ClearsMostFrequentColor()
    {
        ...
        state.SetTile(1, 0, new Tile(4, ElementType.Item1, 1, 0));
        state.SetTile(1, 1, new Tile(5, ElementType.Item1, 1, 1));

        state.SetTile(7, 0, new Tile(6, ElementType.Item3, 7, 0));
        state.SetTile(7, 1, new Tile(7, ElementType.Item3, 7, 1));
        ...
        // 蓝色不应该被清除
        Assert.Equal(ElementType.Item3, state.GetTile(7, 0).Type);
        Assert.Equal(ElementType.Item3, state.GetTile(7, 1).Type);
        Assert.Equal(ElementType.Item3, state.GetTile(7, 2).Type);
    ...
    public void ActivateBomb_UfoBomb_ClearsRandomTile()
    {
        ...
        int finalCount = 0;
        for (int i = 0; i < state.Grid.Length; i++)
            if (state.Grid[i].Type != ElementType.None) finalCount++;
    ...
    public void ProcessSpecialMove_RainbowPlusNormalTile_ClearsColor()
    {
        ...
        // 放置一些红色方块
        state.SetTile(0, 0, new Tile(102, ElementType.Item1, 0, 0));
        ...
        Assert.Equal(ElementType.None, state.GetTile(0, 0).Type);
        Assert.Equal(ElementType.None, state.GetTile(7, 7).Type);
        Assert.Equal(ElementType.None, state.GetTile(3, 3).Type);
        Assert.Equal(ElementType.None, state.GetTile(4, 3).Type);
    }
    ...
    public void ProcessSpecialMove_LineBombPlusLineBomb_ClearsRowAndColumn()
    {
        ...
        for (int y = 0; y < 8; y++)
        {
            Assert.Equal(ElementType.None, state.GetTile(4, y).Type);
        }
    }
    ...
    public void ProcessSpecialMove_HorizontalPlusVerticalRocket_ClearsOnlyOneRowAndOneColumn()
    {
        ...
        for (int x = 0; x < 8; x++)
        {
            Assert.Equal(ElementType.None, state.GetTile(x, 4).Type);
        }
        // 列 x=4 应该全部为 None
        for (int y = 0; y < 8; y++)
        {
    ...
    public void ActivateBomb_ChainExplosion_TriggersNeighborBombs()
    {
        ...
        for (int y = 0; y < 8; y++)
        {
            Assert.Equal(ElementType.None, state.GetTile(6, y).Type);
        }
    }
    ...
    public void ActivateBomb_ChainExplosion_ThreeLevelChain()
    {
        ...
        for (int y = 0; y < 8; y++)
        {
            Assert.Equal(ElementType.None, state.GetTile(5, y).Type);
        }
        // 3. 方块炸弹被触发，消除 (5,6) 周围的 5x5 区域
        // 检查 (4, 5) 应该被清除（在方块炸弹范围内）
        Assert.Equal(ElementType.None, state.GetTile(4, 5).Type);
        Assert.Equal(ElementType.None, state.GetTile(6, 7).Type);
    }
